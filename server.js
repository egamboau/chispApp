...
const rulePositions = (rule) => { if (Array.isArray(rule.positions)) return rule.positions; if (rule.positions) { try { return JSON.parse(rule.positions); } catch {} } return Array.from({ length: rule.endPosition - rule.startPosition + 1 }, (_, index) => rule.startPosition + index); };
const publicRule = (rule) => ({ ...rule, positions: rulePositions(rule) });
const phaseRules = (phaseId) => db.prepare('SELECT * FROM classification_rules WHERE phaseId=? ORDER BY startPosition').all(phaseId).map(publicRule);
function parseRule(body) {
  const start = Number(body.startPosition), end = Number(body.endPosition);
  if (!Array.isArray(body.positions) && (!Number.isInteger(start) || start < 1 || !Number.isInteger(end) || end < start || end - start > 1000)) return null;
  const raw = Array.isArray(body.positions) ? body.positions.map(Number) : Array.from({ length: end - start + 1 }, (_, index) => start + index), positions = [...new Set(raw)].sort((a, b) => a - b), label = clean(body.label);
  return raw.length && raw.every((position) => Number.isInteger(position) && position > 0) && label ? { startPosition: positions[0], endPosition: positions.at(-1), positions, label } : null;
}
const overlaps = (phaseId, rule, except = 0) => phaseRules(phaseId).some((current) => current.id !== except && current.positions.some((position) => rule.positions.includes(position)));
app.get('/api/phases/:id/classification-rules', (req, res) => res.json(phaseRules(req.params.id)));
app.post('/api/phases/:id/classification-rules', (req, res) => { const rule = parseRule(req.body); if (!one('phases', req.params.id)) return fail(res, 404, 'Fase no encontrada.'); if (!rule) return fail(res, 400, 'Regla inválida.'); if (overlaps(req.params.id, rule)) return fail(res, 409, 'Una posición ya pertenece a otra regla.'); const result = db.prepare('INSERT INTO classification_rules(phaseId,startPosition,endPosition,positions,label) VALUES(?,?,?,?,?)').run(req.params.id, rule.startPosition, rule.endPosition, JSON.stringify(rule.positions), rule.label); notify('classification-rules', Number(req.params.id)); res.status(201).json(publicRule(one('classification_rules', result.lastInsertRowid))); });
app.put('/api/phases/:id/classification-rules/:ruleId', (req, res) => { const current = one('classification_rules', req.params.ruleId), rule = parseRule(req.body); if (!current || current.phaseId !== Number(req.params.id)) return fail(res, 404, 'Regla no encontrada.'); if (!rule) return fail(res, 400, 'Regla inválida.'); if (overlaps(req.params.id, rule, current.id)) return fail(res, 409, 'Una posición ya pertenece a otra regla.'); db.prepare('UPDATE classification_rules SET startPosition=?,endPosition=?,positions=?,label=? WHERE id=?').run(rule.startPosition, rule.endPosition, JSON.stringify(rule.positions), rule.label, current.id); notify('classification-rules', current.phaseId); res.json(publicRule(one('classification_rules', current.id))); });
...
const destinationAt = (rules, position) => rules.find((rule) => rulePositions(rule).includes(position))?.label || null;
